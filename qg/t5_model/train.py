...
from qg.t5_model.objects.trainer import Trainer
...
def main(args):
        ...
        ex_context = processed_train_data[0]["context"]
        ex_question = processed_train_data[0]["question"]
        print(ex_context, ex_question)
        ...
        train_sampler = RandomSampler(encoder.encoded_dataset)
        train_dataloader = DataLoader(encoder.encoded_dataset, sampler=train_sampler, batch_size=args.batch_size)
        ...
        results["len_dataset"] = len(processed_train_data)
        results["process_setting"] = args.preprocess_setting
        results["example_context"] = ex_context
        results["example_question"] = ex_question
        ...
        results["epoch_loss"] = trainer.epoch_loss_values
        results["batch_loss"] = trainer.batch_loss
        results["training_epoch_time"] = trainer.epoch_training_time
        results["total_training_time"] = trainer.total_training_time
